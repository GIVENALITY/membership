  ...
  <div class="col-12">
    <div class="card">
      <div class="card-header">
        <h4 class="card-title">Edit Alert: {{ $alert->name }}</h4>
        <p class="card-subtitle text-muted">Modify alert settings and conditions</p>
      </div>
      <div class="card-body">
          ...
          <div class="row mb-4">
            ...
            <div class="col-md-6 mb-3">
              ...
              <input type="text" class="form-control @error('name') is-invalid @enderror" 
                     id="name" name="name" value="{{ old('name', $alert->name) }}" required>
              @error('name')
            ...
            <div class="col-md-6 mb-3">
              ...
              <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                ...
                @foreach($alertTypes as $value => $label)
                  <option value="{{ $value }}" {{ old('type', $alert->type) == $value ? 'selected' : '' }}>
                    {{ $label }}
                  </option>
                @endforeach
              </select>
            ...
            <div class="col-12 mb-3">
              ...
              <textarea class="form-control @error('description') is-invalid @enderror" 
                        ...
                        placeholder="Describe what this alert monitors...">{{ old('description', $alert->description) }}</textarea>
              @error('description')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            </div>
              ...
              <div class="row">
                <div class="col-md-6 mb-3">
                  ...
                  <input type="number" class="form-control" id="spending_amount" name="spending_amount" 
                         value="{{ old('spending_amount', $alert->conditions['amount'] ?? 100000) }}" min="0" step="1000">
                </div>
                <div class="col-md-6 mb-3">
                  ...
                  <select class="form-select" id="spending_period" name="spending_period">
                    <option value="day" {{ old('spending_period', $alert->conditions['period'] ?? 'month') == 'day' ? 'selected' : '' }}>Per Day</option>
                    <option value="week" {{ old('spending_period', $alert->conditions['period'] ?? 'month') == 'week' ? 'selected' : '' }}>Per Week</option>
                    <option value="month" {{ old('spending_period', $alert->conditions['period'] ?? 'month') == 'month' ? 'selected' : 'selected' }}>Per Month</option>
                    <option value="year" {{ old('spending_period', $alert->conditions['period'] ?? 'month') == 'year' ? 'selected' : '' }}>Per Year</option>
                  </select>
                </div>
              </div>
            </div>
              ...
              <div class="row">
                <div class="col-md-6 mb-3">
                  ...
                  <input type="number" class="form-control" id="visit_days" name="visit_days" 
                         value="{{ old('visit_days', $alert->conditions['days'] ?? 30) }}" min="1" max="365">
              ...
              <div class="row">
                <div class="col-md-6 mb-3">
                  ...
                  <input type="number" class="form-control" id="points_amount" name="points_amount" 
                         value="{{ old('points_amount', $alert->conditions['points'] ?? 100) }}" min="0">
                </div>
                <div class="col-md-6 mb-3">
                  ...
                  <select class="form-select" id="points_operator" name="points_operator">
                    <option value="gte" {{ old('points_operator', $alert->conditions['operator'] ?? 'gte') == 'gte' ? 'selected' : 'selected' }}>Greater than or equal to</option>
                    <option value="lte" {{ old('points_operator', $alert->conditions['operator'] ?? 'gte') == 'lte' ? 'selected' : '' }}>Less than or equal to</option>
                    <option value="eq" {{ old('points_operator', $alert->conditions['operator'] ?? 'gte') == 'eq' ? 'selected' : '' }}>Equal to</option>
                  </select>
                </div>
              </div>
            </div>
              ...
              <div class="row">
                <div class="col-md-6 mb-3">
                  ...
                  <input type="number" class="form-control" id="birthday_days" name="birthday_days" 
                         value="{{ old('birthday_days', $alert->conditions['days_ahead'] ?? 7) }}" min="1" max="30">
              ...
              <div class="row">
                <div class="col-md-6 mb-3">
                  ...
                  <input type="number" class="form-control" id="expiry_days" name="expiry_days" 
                         value="{{ old('expiry_days', $alert->conditions['days_ahead'] ?? 30) }}" min="1" max="90">
            ...
            <div class="col-md-6 mb-3">
              ...
              <select class="form-select @error('severity') is-invalid @enderror" id="severity" name="severity" required>
                ...
                @foreach($severityLevels as $value => $label)
                  <option value="{{ $value }}" {{ old('severity', $alert->severity) == $value ? 'selected' : '' }}>
                    {{ $label }}
                  </option>
                @endforeach
              </select>
            ...
            <div class="col-md-6 mb-3">
              ...
              <div class="input-group">
                <input type="color" class="form-control form-control-color" id="color" name="color" 
                       value="{{ old('color', $alert->color) }}" title="Choose alert color">
                <input type="text" class="form-control" value="{{ old('color', $alert->color) }}" 
            ...
            <div class="col-md-6 mb-3">
              <div class="form-check">
                <input class="form-check-input" type="checkbox" id="show_dashboard" name="show_dashboard" 
                       value="1" {{ old('show_dashboard', $alert->show_dashboard) ? 'checked' : '' }}>
            ...
            <div class="col-md-6 mb-3">
              <div class="form-check">
                <input class="form-check-input" type="checkbox" id="show_quickview" name="show_quickview" 
                       value="1" {{ old('show_quickview', $alert->show_quickview) ? 'checked' : '' }}>
            ...
            <div class="col-md-6 mb-3">
              <div class="form-check">
                <input class="form-check-input" type="checkbox" id="send_email" name="send_email" 
                       value="1" {{ old('send_email', $alert->send_email) ? 'checked' : '' }}>
          ...
          <div class="row mb-4" id="email-template-section" style="display: none;">
            ...
            <div class="col-12 mb-3">
              ...
              <textarea class="form-control" id="email_template" name="email_template" rows="5" 
                        placeholder="Custom email template (optional). Use {member_name}, {alert_name}, {triggered_at} as placeholders.">{{ old('email_template', $alert->email_template) }}</textarea>

...
class QRCodeService
{
    ...
    {
        // Create a direct verification URL that opens the member profile
        $verificationUrl = url('/qr-verify/' . $member->membership_id . '/' . $member->hotel_id);
    ...
    public function verifyQRCode(string $qrData): ?array
    {
        try {
            ...
            // Handle both old and new QR data formats
            $membershipId = null;
            $hotelId = null;
            
            // New format (shortened keys)
            if (isset($data['id']) && isset($data['h'])) {
                $membershipId = $data['id'];
                $hotelId = $data['h'];
            }
            // Old format (full keys)
            elseif (isset($data['membership_id']) && isset($data['hotel_id'])) {
            ...
            // For old format, we can trust the data since it was generated by our system
            // For new format, verify the data matches
            if (isset($data['id'])) {
                // New format - verify data integrity
                $expectedData = [
                    't' => 'card',
